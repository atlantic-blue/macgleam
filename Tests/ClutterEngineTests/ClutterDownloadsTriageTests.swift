...
/// C21 lists downloads triage as a ClutterEngine concern and C5 gives the
...
/// their bytes per C5.
@Suite("Clutter scan: downloads triage")
struct ClutterDownloadsTriageTests {
  ...
  func downloadsFilesArePartitionedAcrossTriageFindings() async throws {
    ...
    for downloadsFile in ClutterTree.downloadsPaths {
  ...
  func triageFindingsNeverReachOutsideDownloads() async throws {
    ...
    for finding in outcome.findings(in: .downloadsTriage) {
      ...
      for path in finding.paths {
        #expect(path.isDescendant(of: ClutterFixture.downloadsFolder))
      }
    }
  }
