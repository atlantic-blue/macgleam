...
/// Deterministic fixtures for the leftovers engine tests. Every date and
/// identifier is fixed and nothing reads the wall clock or the real disk.
enum LeftoversFixture {
...
func makeSignedLeftoversCatalog(
  ...
  let signature = try LeftoversSigningFixture.rulesPrivateKey.signature(
...
struct SeededLeftoversFile: Sendable {
  var path: String
  var length: Int
  var created: Date = LeftoversFixture.recentCreatedDate
  var modified: Date = LeftoversFixture.recentModifiedDate
  var lastOpened: Date? = LeftoversFixture.recentOpenedDate
}

/// Seeds an in memory file system with the given files, creating every
/// implied parent directory, with fixed dates throughout.
func makeSeededFileSystem(_ files: [SeededLeftoversFile]) async -> InMemoryFileSystem {
  ...
  for file in files {
    ...
    await fileSystem.seedFile(
      ...
      contents: LeftoversFixture.contents(seedByte, length: file.length),
      isExecutable: false,
...
enum LeftoversTree {
  ...
  static let ancientOpenedDate = LeftoversFixture.daysBeforeReference(400)

  static var files: [SeededLeftoversFile] {
    [
      SeededLeftoversFile(
      ...
      SeededLeftoversFile(
        ...
        lastOpened: LeftoversFixture.daysBeforeReference(Double(oldThresholdDays))),
      SeededLeftoversFile(
        path: oldOneDayNewer, length: 64,
        lastOpened: LeftoversFixture.daysBeforeReference(Double(oldThresholdDays) - 1)),
      SeededLeftoversFile(
        path: oldByModificationFallback, length: 64,
        modified: LeftoversFixture.daysBeforeReference(200),
        lastOpened: nil),
      SeededLeftoversFile(path: freshDocument, length: 32),
      SeededLeftoversFile(path: downloadInstaller, length: 300),
      SeededLeftoversFile(path: downloadReceipt, length: 200),
      SeededLeftoversFile(path: downloadEpisode, length: 150),
      SeededLeftoversFile(
  ...
  static var downloadsPaths: Set<AbsolutePath> {
    [
      LeftoversFixture.path(downloadInstaller),
      LeftoversFixture.path(downloadReceipt),
      LeftoversFixture.path(downloadEpisode),
    ]
  }
  ...
  static func catalog() throws -> RuleCatalog {
    try makeSignedLeftoversCatalog(
...
func makeLeftoversEngine(
  userHome: AbsolutePath = LeftoversFixture.userHome,
  referenceDate: Date = LeftoversFixture.referenceDate
) -> LeftoversEngine {
  LeftoversEngine(userHome: userHome, referenceDate: referenceDate)
}

func makeLeftoversSettings(
  deletionMode: Settings.DeletionMode = .trash,
  largeFileThresholdBytes: UInt64 = LeftoversTree.largeThresholdBytes,
  oldFileThresholdDays: UInt32 = LeftoversTree.oldThresholdDays
) -> Settings {
  Settings(
...
func makeScanContext(
  ...
  rules: RuleCatalog,
  settings: Settings = makeLeftoversSettings(),
  hasFullDiskAccess: Bool = true,
  sessionID: UUID = LeftoversFixture.sessionID
) -> ScanContext {
  ScanContext(
    sessionID: sessionID,
...
func makeLeftoversFinding(
  id: UUID = LeftoversFixture.uuid(0xD1),
  sessionID: UUID = LeftoversFixture.sessionID,
  category: FindingCategory = .largeFile,
  paths: [String] = [LeftoversTree.largeExactlyAtThreshold],
  byteSize: UInt64 = LeftoversTree.largeThresholdBytes,
  ...
  Finding(
    ...
    sessionID: sessionID,
    category: category,
    entries: distributedPathEntries(paths: paths.map(LeftoversFixture.path), byteSize: byteSize),
    risk: risk,
    explanation: "A reviewable leftovers finding used by the plan tests.",
...
func makeExecutor(
  ...
  PlanExecutor(
    ...
    environment: OwnershipEnvironment(
      currentUserHome: LeftoversFixture.userHome, currentUserID: 501),
    now: { LeftoversFixture.executionInstant },
    isCancelled: { false }
  )
}
...
func runLeftoversScan(
  rules: RuleCatalog,
  over fileSystem: InMemoryFileSystem,
  settings: Settings = makeLeftoversSettings(),
  engine: LeftoversEngine = makeLeftoversEngine(),
  sessionID: UUID = LeftoversFixture.sessionID
) async throws -> ScanOutcome {
