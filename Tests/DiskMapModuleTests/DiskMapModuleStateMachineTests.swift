...
struct DiskMapModuleStateMachineTests {
  ...
  func startMappingReadsSettingsAndGrant() async throws {
    let harness = makeDiskMapHarness(deletionMode: .trash, granted: true)
  ...
  func withoutTheGrantTheContextSaysSo() async throws {
    let harness = makeDiskMapHarness(granted: false)
    let feed = await beginMapping(harness)
  ...
  func equalSequencesProduceEqualStates() async throws {
    let first = makeDiskMapHarness()
    let second = makeDiskMapHarness()
...
enum ForeignCommand: Equatable {
  ...
  case executeSelection(expecting: DiskMapCommandRefusal)
  case cancelMapping
