...
final class RefusingFileSystem: FileSystemReading, @unchecked Sendable {
  ...
  private func refuse(_ name: String) {
    ...
    Issue.record("the space lens model touched the file system through \(name)")
  }
...
struct MapFeed: Sendable {
  ...
  let continuation: AsyncThrowingStream<SpaceLensUpdate, Error>.Continuation

  func send(_ updates: SpaceLensUpdate...) {
...
final class FakeSpaceLensMapProvider: SpaceLensMapProviding, @unchecked Sendable {
  ...
  init(module: GleamModule = .spaceLens) {
    self.module = module
  }
  ...
  func map(
    ...
    let (stream, continuation) = AsyncThrowingStream<SpaceLensUpdate, Error>.makeStream()
...
final class FakeSpaceLensSessionProvider: SpaceLensSessionProviding, @unchecked Sendable {
...
final class FakeFullDiskAccessMonitor: FullDiskAccessMonitoring, @unchecked Sendable {
  ...
  @MainActor func openPrivacySettings() {
    ...
    Issue.record("the space lens model must never open privacy settings itself")
  }
}
...
@MainActor
struct SpaceLensHarness {
  let engine: FakeSpaceLensMapProvider
  let executor: FakePlanExecutor
  let store: FakeSettingsStore
  let sessions: FakeSpaceLensSessionProvider
  let access: FakeFullDiskAccessMonitor
  let model: SpaceLensModuleModel
}

@MainActor
func makeSpaceLensHarness(
  ...
  let sessions = FakeSpaceLensSessionProvider(sessionIDs: sessionIDs)
...
@MainActor
func mappingState(_ model: SpaceLensModuleModel) -> SpaceLensMapState? {
  if case .mapping(let map) = model.state { return map }
  return nil
}

@MainActor
func browsingState(_ model: SpaceLensModuleModel) -> SpaceLensMapState? {
  if case .browsing(let map) = model.state { return map }
  return nil
}

@MainActor
func currentMapState(_ model: SpaceLensModuleModel) -> SpaceLensMapState? {
  mappingState(model) ?? browsingState(model)
}

@MainActor
func executionProgress(_ model: SpaceLensModuleModel) -> SpaceLensExecutionProgress? {
  if case .executing(let progress) = model.state { return progress }
  return nil
}

@MainActor
func currentSummary(_ model: SpaceLensModuleModel) -> SpaceLensResultSummary? {
...
struct ModelSnapshot: Equatable {
  let state: SpaceLensModuleState
  let failureNotice: String?
}

@MainActor
func snapshot(_ model: SpaceLensModuleModel) -> ModelSnapshot {
...
func findNode(
  _ root: SpaceLensTreeNode?, at path: AbsolutePath
) -> SpaceLensTreeNode? {
...
func everyNode(_ root: SpaceLensTreeNode?) -> [SpaceLensTreeNode] {
...
@MainActor
func reachBrowsing(_ harness: SpaceLensHarness) async -> SpaceLensMapState? {
