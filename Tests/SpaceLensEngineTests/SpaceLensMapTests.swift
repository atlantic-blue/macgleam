...
struct SpaceLensMapStreamingTests {

  @Test("the engine names the space lens module")
  func engineNamesTheSpaceLensModule() {
    #expect(SpaceLensEngine().module == .spaceLens)
  }
  ...
  func everySeededPathArrivesAsANode() async throws {
    ...
    let expected: Set<AbsolutePath> = Set(
      LensTree.files.map { SpaceLensFixture.path($0.path) } + [
        SpaceLensFixture.path(LensTree.documentsDirectory),
        SpaceLensFixture.path(LensTree.reportsDirectory),
        SpaceLensFixture.path(LensTree.picturesDirectory),
        SpaceLensFixture.path(LensTree.protectedDirectory),
        SpaceLensFixture.path("/Users/lens/Library"),
        SpaceLensFixture.path("/Users/lens"),
        SpaceLensFixture.path("/Users"),
      ])
  ...
  func nodesLinkToTheirParents() async throws {
    ...
    let root = try #require(outcome.node(at: SpaceLensFixture.volumeRoot))
    #expect(root.parent == nil)
    #expect(root.isDirectory)

    let reports = try #require(
      outcome.node(at: SpaceLensFixture.path(LensTree.reportsDirectory)))
    #expect(reports.parent == SpaceLensFixture.path(LensTree.documentsDirectory))
    #expect(reports.isDirectory)

    let q1 = try #require(outcome.node(at: SpaceLensFixture.path(LensTree.reportQ1)))
    #expect(q1.parent == SpaceLensFixture.path(LensTree.reportsDirectory))
...
struct SpaceLensMapTotalTests {
  ...
  func fileNodeTotalIsItsAllocatedBytes() async throws {
    ...
    let photoPath = SpaceLensFixture.path(LensTree.photo)
  ...
  func rootTotalCoversTheWholeVolume() async throws {
    ...
    #expect(
      outcome.finalTotals[SpaceLensFixture.volumeRoot] == expected[SpaceLensFixture.volumeRoot])
  }
...
struct SpaceLensMapSelectabilityTests {
  ...
  func denylistedDirectoryRendersUnselectable() async throws {
    ...
    let protected = try #require(
      outcome.node(at: SpaceLensFixture.path(LensTree.protectedDirectory)))
  ...
  func denylistedDescendantIsUnselectable() async throws {
    ...
    let keychain = try #require(
      outcome.node(at: SpaceLensFixture.path(LensTree.protectedKeychain)))
  ...
  func unblockedPathsAreSelectable() async throws {
    ...
    for path in [LensTree.documentsDirectory, LensTree.reportQ1, LensTree.photo] {
      let node = try #require(outcome.node(at: SpaceLensFixture.path(path)))
...
struct SpaceLensMapCancellationTests {
  ...
  func cancellingTheConsumerEndsTheStreamPromptly() async throws {
    ...
    let consumer = Task {
      var received = 0
      do {
        for try await update in SpaceLensEngine().map(
          volume: SpaceLensFixture.volumeRoot, context: context)
        {
          if case .node = update {
            received += 1
            if received == 3 {
