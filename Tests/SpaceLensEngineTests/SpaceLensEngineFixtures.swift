...
/// Deterministic fixtures for the space lens engine tests. Every date and
/// identifier is fixed and nothing reads the wall clock or the real disk.
enum SpaceLensFixture {
...
func makeSignedSpaceLensCatalog(
  ...
  let signature = try SpaceLensSigningFixture.rulesPrivateKey.signature(
...
func makeSeededFileSystem(_ files: [SeededLensFile]) async -> InMemoryFileSystem {
  ...
  for file in files {
    ...
    await fileSystem.seedFile(
      ...
      contents: SpaceLensFixture.contents(seedByte, length: file.length),
      isExecutable: false,
      created: SpaceLensFixture.createdDate,
      modified: SpaceLensFixture.modifiedDate,
      lastOpened: nil,
      extendedAttributes: [:]
    )
    seedByte &+= 1
...
enum LensTree {
  ...
  static func catalog() throws -> RuleCatalog {
    try makeSignedSpaceLensCatalog(
...
func makeExecutor(
  ...
  PlanExecutor(
    ...
    environment: OwnershipEnvironment(
      currentUserHome: SpaceLensFixture.path("/Users/lens"), currentUserID: 501),
    now: { SpaceLensFixture.executionInstant },
    isCancelled: { false }
  )
}
...
func makeScanContext(
  ...
  rules: RuleCatalog,
  settings: Settings = makeSpaceLensSettings(),
  hasFullDiskAccess: Bool = true,
  sessionID: UUID = SpaceLensFixture.sessionID
) -> ScanContext {
  ScanContext(
    sessionID: sessionID,
...
/// A space lens selection finding as C22 mints it: category
/// spaceLensSelection, risk review, never preselected, one entry per node.
func makeSpaceLensSelection(
  id: UUID = SpaceLensFixture.uuid(0xF1),
  sessionID: UUID = SpaceLensFixture.sessionID,
  entries: [PathEntry]
) -> Finding {
  Finding(
...
struct MapOutcome: Sendable {
  var updates: [SpaceLensUpdate] = []
  var nodesInOrder: [SpaceLensNode] = []
  ...
  func node(at path: AbsolutePath) -> SpaceLensNode? {
...
func runMap(
  over fileSystem: InMemoryFileSystem,
  rules: RuleCatalog,
  volume: AbsolutePath = SpaceLensFixture.volumeRoot,
  sessionID: UUID = SpaceLensFixture.sessionID
) async throws -> MapOutcome {
  let context = makeScanContext(over: fileSystem, rules: rules, sessionID: sessionID)
  return try await collectMap(SpaceLensEngine().map(volume: volume, context: context))
}

// MARK: - True totals

func componentAncestors(of path: AbsolutePath) -> [AbsolutePath] {
  var ancestors: [AbsolutePath] = [SpaceLensFixture.path("/")]
  var current = ""
...
func trueSubtreeTotals(
  ...
  var totals: [AbsolutePath: UInt64] = [SpaceLensFixture.volumeRoot: 0]
  for try await event in fileSystem.enumerate(
    root: SpaceLensFixture.volumeRoot, options: options)
  {
...
func allocatedBytes(
  ...
  try await fileSystem.metadata(at: SpaceLensFixture.path(path)).allocatedBytes
}
