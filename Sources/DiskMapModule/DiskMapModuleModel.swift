...
/// interplay: Disk Map has no hub card, contributes nothing to the hub's
...
public final class DiskMapModuleModel {
  ...
  public init(
    ...
    precondition(
      engine.module == .diskMap,
      "DiskMapModuleModel requires the disk map engine, not \(engine.module)."
    )
    self.engine = engine
  ...
  public func drillIn(to path: AbsolutePath) -> DiskMapDrill? {
    ...
    focusPath = path
    publishMap()
    return DiskMapDrill(target: path, direction: .zoomIn)
  }
  ...
  public func drillOut() -> DiskMapDrill? {
    ...
    focusPath = parent
    publishMap()
    return DiskMapDrill(target: parent, direction: .zoomOut)
  }
  ...
  private func apply(_ update: DiskMapUpdate) {
  ...
  /// selection.
  private func reconcileIntentions(with node: DiskMapNode) {
  ...
  /// Mints one finding per selected node: category diskMapSelection, risk
  ...
  private func updateExecutionProgress(
    _ transform: (DiskMapExecutionProgress) -> DiskMapExecutionProgress
  ) {
  ...
  private func finishExecution(
    ...
    let summary = DiskMapResultSummary(report: report, plan: plan, refusals: refusals)
    discardMap()
...
private struct MapTree {
  ...
  mutating func absorb(_ node: DiskMapNode) {
  ...
  func buildRoot() -> DiskMapTreeNode? {
    buildNode(at: volume)
  }

  private func buildNode(at path: AbsolutePath) -> DiskMapTreeNode? {
...
extension DiskMapResultSummary {
...
extension DiskMapExecutionProgress {
  fileprivate func starting(operationID: UUID) -> DiskMapExecutionProgress {
    DiskMapExecutionProgress(
      planID: planID,
  ...
  fileprivate func finishing(result: OperationResult) -> DiskMapExecutionProgress {
    var reclaimed = bytesReclaimed
    if case .completed(let bytes) = result {
      reclaimed += bytes
    }
    return DiskMapExecutionProgress(
      planID: planID,
      totalOperations: totalOperations,
